...
(function() {
  ...
  SWD_Page_Recorder = (function() {
    ...
    SWD_Page_Recorder.dragging = false;

    SWD_Page_Recorder.dragTop = 0;

    SWD_Page_Recorder.dragLeft = 0;

    SWD_Page_Recorder.prototype.doDragging = function(e) {
      var currentX, currentY, dragLeft, dragTop, error, left, parent_win, top;
      if (!this.Draggable) {
        return;
      }
      if (!this.dragging) {
      ...
      parent_win = this.getMainWinElement();
      ...
      top = currentY > dragTop ? top + currentY - dragTop : top - dragTop - currentY;
      left = currentX > dragLeft ? left + currentX - dragLeft : left - dragLeft - currentX;
      dragTop = currentY;
      dragLeft = currentX;
      try {
        parent_win.style.top = top + "px";
        return parent_win.style.left = left + "px";
      } catch (_error) {
        error = _error;
        return console.log("SWD Error: " + error);
      }
    };

    SWD_Page_Recorder.prototype.startDragging = function(e) {
      if (!this.Draggable) {
        return;
      }
      this.dragging = true;
      this.dragTop = e.pageY;
      this.dragLeft = e.pageX;
      return this.getMainWinElement().style.cursor = "move";
    };

    SWD_Page_Recorder.prototype.stopDragging = function() {
      if (!this.Draggable) {
        return;
      }
      this.dragging = false;
      return this.getMainWinElement().style.cursor = "default";
    };
